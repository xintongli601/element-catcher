...
- Milestone 6C - Safe generated-component preview for Previewable Subset V1: Current.
...
Acceptance status: Current. Milestone 6A is completed as the architecture and threat model review. Milestone 6B is completed as the accepted sandbox runtime foundation: the Side Panel trusted extension page owns two sibling packaged sandbox frames, a packaged sandbox host and a packaged sandbox render realm, and uses a narrow trusted relay with strict source-window, exact-key, message-size, requestId, sessionNonce, fixtureId, and lifecycle validation. Milestone 6C adds production safe generated-component preview on top of that sibling foundation. The Side Panel exposes an explicit Preview action for each generated version, keeps the generated source visible and copyable, computes and validates source identity before mounting preview frames, and starts a fresh requestId, sessionNonce, host frame, and render frame per attempt. The host receives bounded generated source, parses it with the packaged `@babel/parser` dependency, accepts only Previewable Subset V1, and converts approved JSX into a plain-data `PreviewRenderPlanV1`. The render realm never receives generated source and independently validates the render plan plus canonical plan hash before rendering through React `createRoot`. Approved class tokens render through a source-controlled bounded utility stylesheet kept in exact parity with the class-token registry; Tailwind itself is not run. Unsupported or unsafe source remains source-only and does not mutate the generated version or source CaptureRecord. Protocol V2 replaces the former fixture protocol with direction-specific message types for host init, render init, source request, plan success/failure, render plan, render success/failure, and dispose. Timeout and every terminal failure dispose both sandbox frames through React-controlled unmount, and reopen creates fresh requestId, sessionNonce, and WindowProxy identities. Async host, render, and trusted validation operations use stale-operation guards so late parser/hash continuations cannot reverse terminal state. Trusted message handling checks current `WindowProxy` identity before byte-size or schema traversal. Milestone 6C intentionally does not implement natural-language revision, regeneration management, version comparison, export, storage migrations, backend contract changes, provider calls, arbitrary JavaScript execution, Tailwind compilation, external assets, hooks, imports, browser APIs, workers, storage, navigation, or network access from generated source. Final validation for the 6C production implementation must pass before 6C can be marked Completed.
